...
//    private static JdbcTemplate jdbcTemplate = context.getBean(JdbcTemplate.class);
    ...
    public Person findById(int id) {
        Person person = new Person();
        jdbcTemplate.query("SELECT * FROM PERSONS WHERE id=" + id, rs -> {
            person.setId(rs.getInt("ID"));
            person.setFirstName(rs.getString("FIRST_NAME"));
            person.setLastName(rs.getString("LAST_NAME"));
            person.setSalary(rs.getInt("SALARY"));
    ...
    public void update(Person person) {
        jdbcTemplate.update(conn -> {
            PreparedStatement ps = conn.prepareStatement("UPDATE PERSONS SET FIRST_NAME=?, LAST_NAME=?, SALARY=? WHERE ID=?");
            ps.setString(1, person.getFirstName());
            ps.setString(2, person.getLastName());
            ps.setInt(3, person.getSalary());
